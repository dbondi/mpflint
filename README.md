...
**Why it helps.** An observable effect of predicted_swap is that the H diagonal entries become more balanced after each pass — the spread between the largest and smallest diagonal magnitudes decreases. This is interesting because strategies that directly optimize for diagonal balance (e.g., minimizing max/min ratio, equalizing log-diag values) were tested and performed worse. The diagsum criterion with Givens simulation achieves balancing as a side effect rather than as an explicit goal, and this indirect approach appears to be more effective — possibly because it respects the rotational structure of H rather than treating the diagonal entries as independent values.

**Related work in lattice reduction.** The idea of simulating a swap before committing exists in the LLL lattice reduction literature. [GfcLLL](https://ar5iv.labs.arxiv.org/html/1607.01064) (Jaldén & Elia, 2016) uses the same Givens prediction mathematics to evaluate adjacent swap pairs, selecting the single globally best one per iteration. [SS-LLL](https://eprint.iacr.org/2023/261) (Ryan et al., 2023) extends this to deep insertions, scoring all O(n²) candidate positions by decrease in sum-of-squared Gram-Schmidt norms. [PotLLL](https://arxiv.org/abs/1212.5100) (Fontein et al., 2014) gates deep insertions on whether the basis potential actually decreases.

However, these approaches all perform a single operation per iteration. In our testing, applying their criteria as single-swap-per-iteration strategies gave <2% improvement over standard PSLQ — one swap per iteration is not enough. Running their approach as intended — repeatedly picking the globally best swap until no improvement remains — produces ~4× iteration reduction, close to predicted_swap but 5–7% worse. The difference is in how swaps are selected: their repeated global search re-evaluates all O(n) pairs after each swap, while our bidirectional insertion sort walks through positions sequentially, which appears to find a slightly better ordering by respecting locality.
...
**phi_s29 cumulative speedup (n=197, no IP available), all on M1 Max 64 GB:**
