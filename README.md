...
However, these approaches all perform a single operation per iteration. In our testing, applying their criteria as single-swap-per-iteration strategies gave <2% improvement over standard PSLQ. The iteration reduction only appears when the criterion is applied across *multiple swaps per iteration* via bidirectional insertion sort. When we adapted GfcLLL's Babai metric, the SS criterion, and PotLLL's potential criterion to our multi-swap insertion sort structure, all produced similar iteration counts (~4× reduction) — nearly matching predicted_swap. The acceptance criterion matters less than the multi-swap structure.
