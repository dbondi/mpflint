...
We implemented these criteria in our PSLQ context. When applied as a repeated global best pick — scanning all pairs, executing the best swap, re-scanning until no improvement remains — they produce ~4× iteration reduction, comparable to predicted_swap but 5–7% worse. The Babai metric (GfcLLL), sum-of-squares (SS-LLL), and potential (PotLLL) criteria all converge to nearly identical iteration counts. The difference between these and predicted_swap is in the selection structure: their global re-scan after each swap vs our bidirectional insertion sort, which walks through positions sequentially and appears to find a slightly better ordering.
